...
function Stepper({
  ...
  const focusLast = React.useCallback(
    () => focusTrigger(Math.max(0, triggerNodes.length - 1)),
    [focusTrigger, triggerNodes.length]
  );
  ...
  const contextValue = React.useMemo<StepperContextValue>(
    ...
    [currentStep, handleSetActiveStep, children, orientation, registerTrigger, triggerNodes, indicators, focusNext, focusPrev, focusFirst, focusLast]
  );
  ...
  return (
    <StepItemContext.Provider value={itemContextValue}>
      <div
        ...
        className={cn(
          'group/step flex items-center group-data-[orientation=horizontal]/stepper-nav:flex-row group-data-[orientation=vertical]/stepper-nav:flex-col',
          className
        )}
        data-state={state}
...
function StepperTrigger({ asChild = false, className, children, tabIndex, ...props }: StepperTriggerProps) {
  ...
  return (
    <button
      ...
      className={cn(
        'inline-flex cursor-default items-center gap-3 rounded-full outline-none focus-visible:z-10 focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-60',
        className
      )}
...
function StepperSeparator({ className }: React.ComponentProps<'div'>) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        'm-0.5 rounded-full bg-border group-data-[orientation=vertical]/stepper-nav:h-12 group-data-[orientation=vertical]/stepper-nav:w-0.5 group-data-[orientation=horizontal]/stepper-nav:h-0.5 group-data-[orientation=horizontal]/stepper-nav:min-w-4 group-data-[orientation=horizontal]/stepper-nav:flex-1',
        'group-data-[state=completed]/step:flex-1',
        className
      )}
    />
  );
  ...
  return (
    <h3
      ...
      className={cn(
        ...
        'data-[state=active]:text-foreground data-[state=inactive]:text-foreground-muted data-[state=completed]:text-foreground-secondary',
        className
      )}
    >
      {children}
...
function StepperNav({ children, className }: React.ComponentProps<'nav'>) {
  ...
  return (
    <nav
      ...
      data-orientation={orientation}
      className={cn(
        'group/stepper-nav inline-flex w-full data-[orientation=horizontal]:flex-row data-[orientation=vertical]:flex-col',
        className
      )}
    >
      {children}
