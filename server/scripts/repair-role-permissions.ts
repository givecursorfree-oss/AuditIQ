 ...
 * Fixes Partner accounts stuck with Staff-level permission keys.
 *
...
async function ensurePartnerRole(allIds: string[]): Promise<string> {
  ...
  await prisma.rolePermission.createMany({
    data: allIds.map((permissionId) => ({ roleId: role.id, permissionId })),
  });
  console.log(`✅ Partner: ${allIds.length} permission(s) (role ${role.id})`);
  return role.id;
}
...
async function setRolePermissions(roleName: string, permissionIds: string[]) {
  ...
  if (!role) {
    if (roleName === 'Partner') {
      await ensurePartnerRole(permissionIds);
      return;
    }
    console.warn(`⚠️  Role "${roleName}" not found — skip`);
    return;
  }
  await prisma.rolePermission.deleteMany({ where: { roleId: role.id } });
